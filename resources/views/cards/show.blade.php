@php
    ...
    $fullName = $card->full_name ?: $card->company_name ?: 'Электронная визитка';
    ...
    foreach ($contacts as $contact) {
        $isSocial = ContactChannelRegistry::isMessenger($contact);
        $lastIndex = count($contactGroups) - 1;

        if ($isSocial && $lastIndex >= 0 && $contactGroups[$lastIndex]['type'] === 'social') {
            $contactGroups[$lastIndex]['items'][] = $contact;
        } else {
            $contactGroups[] = [
                'type' => $isSocial ? 'social' : 'contact',
                'items' => [$contact],
            ];
        }
    }
    ...
    $routeName = fn (string $name): string => config('digital-business-cards.route_name_prefix', 'cards.').$name;
@endphp
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <link rel="stylesheet" href="{{ config('digital-business-cards.assets_url') ? asset(trim(config('digital-business-cards.assets_url'), '/').'/card.css') : route(config('digital-business-cards.route_name_prefix', 'cards.').'assets', ['file' => 'card.css']) }}">
    <script type="module" src="{{ config('digital-business-cards.assets_url') ? asset(trim(config('digital-business-cards.assets_url'), '/').'/card.js') : route(config('digital-business-cards.route_name_prefix', 'cards.').'assets', ['file' => 'card.js']) }}"></script>
</head>
...
<main class="digital-card-shell" data-digital-card data-events-url="{{ route($routeName('events.store'), $card) }}">
    <section class="digital-card-hero" @if($card->cover_image) style="background-image:linear-gradient(180deg,rgba(8,13,26,.15),var(--card-bg) 94%),url('{{ $card->storageUrl($card->cover_image) }}')" @endif>
        @if ($card->logo)
            <div class="digital-card-topbar">
                <a href="{{ $card->publicUrl() }}" aria-label="{{ $card->company_name ?: $fullName }}">
                    <img src="{{ $card->storageUrl($card->logo) }}" alt="{{ $card->company_name ?: 'Логотип' }}" class="digital-card-logo">
                </a>
            </div>
        @endif
        <div class="digital-card-profile">
            @if ($card->avatar)
                <button type="button" class="digital-card-avatar-button" data-open-image aria-label="Открыть фотографию {{ $fullName }}">
        ...
        <div class="digital-card-primary-actions">
            <button type="button" class="digital-card-save {{ $buttonClass }}" data-save-contact data-track="vcard"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 21h14"/></svg>Сохранить в контакты</button>
        </div>
    </section>

    <div class="digital-card-content">
        @if ($contacts)
            <section class="digital-card-section digital-card-contacts-section" aria-label="Способы связи">
                ...
                @foreach ($contactGroups as $group)
                    @if ($group['type'] === 'social')
                        ...
                        @endphp
                        <div class="digital-card-social-row digital-card-social-row--{{ $socialCount === 1 ? 'single' : 'multiple' }}" style="--social-count: {{ $socialCount }}" aria-label="Социальные сети и мессенджеры">
        ...
        @if ($card->blocks->isNotEmpty())<section class="digital-card-section"><h2>Материалы и ссылки</h2><div class="digital-card-blocks">@foreach ($card->blocks as $block)
            @php($data = $block->data ?: []) @php($media = $card->storageUrl($data['media'] ?? null)) @php($target = ($data['open_in_new_tab'] ?? true) ? '_blank' : '_self')
            @if (in_array($block->type, ['link', 'social']))<a href="{{ $block->url }}" target="{{ $target }}" @if($target === '_blank') rel="noopener noreferrer" @endif class="digital-card-link-block {{ $buttonClass }}" data-track="cta" data-block-id="{{ $block->id }}"><span><strong>{{ $block->button_label ?: $block->title ?: 'Открыть' }}</strong>@if($block->content)<small>{{ $block->content }}</small>@endif</span><span>›</span></a>
            ...
            @elseif ($block->type === 'gallery')<article class="digital-card-gallery-block">@if($block->title)<h3>{{ $block->title }}</h3>@endif @if($block->content)<p>{{ $block->content }}</p>@endif<div class="digital-card-gallery">@foreach (($data['gallery'] ?? []) as $image)<a href="{{ $card->storageUrl($image) }}" target="_blank" rel="noopener"><img src="{{ $card->storageUrl($image) }}" alt="{{ $block->title ?: 'Фото '.$loop->iteration }}" loading="lazy"></a>@endforeach</div></article>
            @elseif ($block->type === 'video')<article class="digital-card-media-block">@if($block->title)<h3>{{ $block->title }}</h3>@endif @if($block->content)<p>{{ $block->content }}</p>@endif @if($block->url)<a href="{{ $block->url }}" target="{{ $target }}" @if($target === '_blank') rel="noopener noreferrer" @endif class="digital-card-video-link" data-track="video" data-block-id="{{ $block->id }}">▶ Смотреть видео</a>@endif</article>
            @elseif ($block->type === 'file')<a href="{{ $media ?: $block->url }}" target="_blank" rel="noopener noreferrer" class="digital-card-file-block" data-track="file" data-block-id="{{ $block->id }}"><span>⇩</span><span><strong>{{ $block->title ?: 'Скачать файл' }}</strong>@if($block->content)<small>{{ $block->content }}</small>@endif</span></a>
    ...
    <footer class="digital-card-footer"><span>Электронная визитка</span></footer>
</main>
@if ($card->avatar)
<div class="digital-card-modal digital-card-image-modal" data-modal="image" hidden>
    ...
    <section class="digital-card-lightbox" role="dialog" aria-modal="true" aria-label="Фотография {{ $fullName }}">
        <button type="button" class="digital-card-lightbox-close" data-close-modal aria-label="Закрыть полноэкранное изображение">×</button>
        <img src="{{ $card->storageUrl($card->avatar) }}" alt="{{ $fullName }}">
    </section>
</div>
@endif
<div class="digital-card-modal" data-modal="save" hidden><div class="digital-card-modal-backdrop" data-close-modal></div><section class="digital-card-dialog digital-card-save-dialog" role="dialog" aria-modal="true" aria-labelledby="save-title"><button type="button" class="digital-card-modal-close" data-close-modal aria-label="Закрыть">×</button><h2 id="save-title">Выберите удобный способ получения визитки</h2>@foreach ($contacts as $contact) @php($type = $contact['type'] ?? '') @if(in_array($type, ['telegram', 'max'], true))<a href="{{ ContactChannelRegistry::href($contact) }}" target="_blank" rel="noopener noreferrer" class="digital-card-delivery"><x-digital-business-cards::contact-icon :type="$type" /><span>Отправить в {{ $type === 'max' ? 'MAX' : 'Telegram' }}</span></a>@endif @endforeach<a href="{{ route($routeName('download'), $card) }}" data-download-vcard class="digital-card-save digital-card-save-dialog-vcard {{ $buttonClass }}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 21h14"/></svg><span>Сохранить в контакты</span></a>@if ($card->lead_form_enabled)<button type="button" data-open-exchange data-track="contact" class="digital-card-exchange {{ $buttonClass }}">Обменяться контактами</button>@endif</section></div>
@if ($card->lead_form_enabled)<div class="digital-card-modal" data-modal="exchange" @unless($shouldOpenExchange) hidden @endunless><div class="digital-card-modal-backdrop" data-close-modal></div><section class="digital-card-dialog" role="dialog" aria-modal="true" aria-labelledby="exchange-title"><button type="button" class="digital-card-modal-close" data-close-modal aria-label="Закрыть">×</button>
...
@if (session('card_lead_sent'))
    <div class="digital-card-modal" data-modal="success">
        ...
        <section class="digital-card-dialog digital-card-success-dialog" role="dialog" aria-modal="true" aria-labelledby="exchange-success-title" aria-describedby="exchange-success-description">
            <button type="button" class="digital-card-modal-close" data-close-modal aria-label="Закрыть">×</button>
            <h2 id="exchange-success-title">Ваши контакты успешно отправлены</h2>
            <p id="exchange-success-description">
                @if(session('card_confirmation_sent'))
                    На вашу электронную почту отправлены контакты «{{ $fullName }}»
                @else
                    «{{ $fullName }}» получил(а) ваши контактные данные
                @endif
            </p>
            <button type="button" class="digital-card-submit card-button-pill" data-close-modal>OK</button>
        </section>
    </div>
@endif
</body></html>
