...
<form wire:submit="submit" class="digital-card-form" novalidate @if($inline) data-inline-lead-form @endif>
    ...
    @foreach ($fieldDefinitions as $field)
        ...
        <label wire:key="lead-field-{{ $key }}">
            ...
            @if(($field['type'] ?? 'text') === 'textarea')
                <textarea wire:model.live.blur="fields.{{ $key }}" name="{{ $key }}" rows="3" @required($field['required'] ?? false)></textarea>
            @else
                <input wire:model.live.blur="fields.{{ $key }}" type="{{ $field['type'] ?? 'text' }}" name="{{ $key }}" @required($field['required'] ?? false)>
            @endif
            @error('fields.'.$key)
                <em>{{ $message }}</em>
            @enderror
    ...
    <label class="digital-card-consent">
        <input wire:model.live.change="consent" type="checkbox" name="consent" value="1" @required($card->lead_consent_required)>
        <span>
