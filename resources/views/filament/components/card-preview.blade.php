@php
    ...
    /** @var \DigitalCardKit\Laravel\Models\DigitalBusinessCard $card */
...
@endphp
{{-- The stylesheet is the same one the public site loads, so the preview is
     pixel-identical to the live card. Linked here (not via the route <head>)
     because the preview is a fragment rendered inside a Filament modal. --}}
<link rel="stylesheet" href="{{ $assetUrl('card.css') }}">
<p class="digital-card-preview-notice">{{ __('digital-business-cards::admin.cards.preview.notice') }}</p>
<div
...
<main class="digital-card-shell">
    <section class="digital-card-hero" @if($card->cover_image) style="background-image:linear-gradient(180deg,rgba(8,13,26,.15),var(--card-bg) 94%),{{ \DigitalCardKit\Laravel\Support\Css::url((string) $card->storageUrl($card->cover_image)) }}" @endif>
        @if ($card->logo)
        ...
        <div class="digital-card-primary-actions">
            <button type="button" class="digital-card-save {{ $buttonClass }}" disabled style="opacity:.5;cursor:not-allowed"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 21h14"/></svg>{{ $t('actions.save_contact') }}</button>
        </div>
    </section>
